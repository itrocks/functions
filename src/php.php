 ...
 * @link https://it.rocks/reference/functions/php
 */
 ...
 *
 * @param $memory_limit integer|string php memory limit in o / given unit
 ...
 */
function upgradeMemoryLimit(int|string $memory_limit) : bool
{
	$old_memory_limit = ini_get('memory_limit');
	$multipliers      = ['G' => 1024 * 1024 * 1024, 'M' => 1024 * 1024, 'K' => 1024];
	// convert $memory_limit to octets
	$unit         = substr($memory_limit, -1);
	$memory_limit = isset($multipliers[$unit])
		? intval(substr($memory_limit, 0, -1)) * $multipliers[$unit]
		: intval($memory_limit);
	// convert old_memory_limit to octets
	$unit             = intval(substr($old_memory_limit, -1));
	$old_memory_limit = isset($multipliers[$unit])
		? intval(substr($old_memory_limit, 0, -1)) * $multipliers[$unit]

 ...
 * @link https://it.rocks/reference/functions/type
 */
 ...
 * @return object|null
 */
function instanceIn(object|string $class_name_or_object, array $objects) : object|null
{
 ...
 * @param $class_name object|object[]|string|string[] If multiple, result is true for any of them
 * @return boolean
 */
$GLOBALS['final_class_function'] = null;
function isA(object|string $object, array|object|string $class_name) : bool
{
	global $final_class_function;
	if (is_array($class_name)) {
		...
		if (is_string($object)) {
			if ($final_class_function) {
				$object = call_user_func($final_class_function, $object);
			}
		}
 ...
 * @return boolean
 */
function isStrictInteger(string $value) : bool
{
	...
	: bool
{
	...
	if ((is_float($value) && !$decimal_allowed) || !is_numeric($value)) {
		return false;
	}
 ...
 */
function maxSet(array|float|int $arguments) : int|null
{
	$maximum = null;
 ...
 * @return integer|null null if there is not any real value into arguments
 */
function minSet(array|float|int $arguments) : int|null
{
	$minimum = null;
	foreach (func_get_args() as $argument) {
		if (is_array($argument)) {
 ...
 *
 * @param $object object|string
 * @param $filter integer @values | T_CLASS, T_INTERFACE, T_TRAIT
 * @return string[]
 */
function parents(object|string $object, int $filter = T_CLASS | T_INTERFACE | T_TRAIT) : array
{
	if (is_object($object)) {
		$object = get_class($object);
	}
	...
	$result  = ($filter & T_CLASS) ? $parents : [];

	do {
		$next_classes = [];
		foreach ($classes as $class) {
			if ($filter & T_INTERFACE) {
				$next_classes += class_implements($class);
			}
			if ($filter & T_TRAIT) {
				$next_classes += class_uses($class);
			}
